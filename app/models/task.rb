class Task < ApplicationRecord
  ...
  scope :weekly_tasks, -> (timezone) {where("Date(task_date) >= ?", Time.now.weeks_ago(1).to_date)}
  scope :monthly_tasks, -> (timezone) {where("Date(task_date) >= ?", Time.now.months_ago(1).to_date)}
