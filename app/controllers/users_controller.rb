class UsersController < ApplicationController
  ...
  def new
    ...
    @user.tasks.create(task_date: Time.current.in_time_zone(@user.get_timezone).to_date, status: false) unless @user.tasks.present?
    @grouped_tasks = @user.tasks.where(task_date: Time.current.in_time_zone(@user.get_timezone).to_date)
  ...
  def stats
    @user = User.find(params[:user_id])
    @completed_tasks_today = @user.tasks.active_tasks.current_tasks(Time.current.in_time_zone(@user.get_timezone).to_date).completed_tasks.count
    @completed_tasks_week = @user.tasks.active_tasks.weekly_tasks(@user.get_timezone).completed_tasks.count
    @completed_tasks_month = @user.tasks.active_tasks.monthly_tasks(@user.get_timezone).completed_tasks.count
    ...
    if params["daterange"].present?
      ...
      @date_from = (Time.current.in_time_zone(@user.get_timezone)-29.days).to_date
      @date_to = Time.current.in_time_zone(@user.get_timezone).to_date
    end
